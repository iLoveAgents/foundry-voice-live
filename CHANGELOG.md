...
- A user turn submitted while tool executors are still running is **handed over to the tool batch** rather than answered on `response.done`: sending it earlier made the model reply to a conversation whose `function_call_output` did not exist yet. One follow-up now covers the tool result and that turn together — and still happens when every executor returns void.
